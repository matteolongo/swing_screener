...
- `intelligence/`: runtime intelligence snapshots, jobs, caches, and reports
  ...
  - `intelligence/runs/index/{TICKER}.json`: per-symbol run-trace index — newest-first list of `{run_id, generated_at}` entries capped at `config.tracing.max_runs_per_ticker` (default 50). Regenerable, gitignored. Accessed via `GET /api/intelligence/{ticker}/runs`.
