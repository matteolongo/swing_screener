...
- `intelligence/`: runtime intelligence snapshots, jobs, caches, and reports
  ...
  - `intelligence/runs/{run_id}.json`: per-run agent trace — step-by-step execution log for a single analysis run. Each run record contains status, timing, model, token usage, source counts, prompt hash, and error details. Regenerable, gitignored. No schema migration required. Accessed via `GET /api/intelligence/runs/{run_id}`.
  - `intelligence/runs/index/{TICKER}.json`: per-symbol run-trace index — newest-first list of `{run_id, ticker, started_at, finished_at, status, duration_ms, step_count}` entries capped at `config.tracing.max_runs_per_ticker` (default 50). Regenerable, gitignored. Accessed via `GET /api/intelligence/{ticker}/runs`.
