...
def normalize_ohlcv(ohlcv: pd.DataFrame) -> pd.DataFrame:
    """Validate and normalize canonical ``(field, ticker)`` OHLCV data."""
    if not isinstance(ohlcv, pd.DataFrame):
        raise ValueError("OHLCV must be a pandas DataFrame.")
    if not isinstance(ohlcv.columns, pd.MultiIndex) or ohlcv.columns.nlevels != 2:
        raise ValueError(
            "OHLCV must have two-level MultiIndex columns (field, ticker)."
        )
    if ohlcv.columns.duplicated().any():
        raise ValueError("OHLCV contains duplicate (field, ticker) columns.")
    if "Close" not in ohlcv.columns.get_level_values(0):
        raise ValueError("Field 'Close' not found in OHLCV.")
    try:
        dates = pd.to_datetime(ohlcv.index, errors="raise")
    except (TypeError, ValueError) as exc:
        raise ValueError("OHLCV index contains an invalid date.") from exc

    normalized = ohlcv.copy()
    normalized.index = pd.DatetimeIndex(dates)
