...
def test_screener_fallback_stop_uses_strategy_atr_multiplier(monkeypatch):
    """When the report has no explicit stop, fallback stop honors risk.k_atr."""
    ohlcv = _ohlcv_with_spy()
    mock_provider = _create_mock_provider(ohlcv)
...
def test_screener_keeps_missing_sma_fields_null(monkeypatch):
    """Missing SMA inputs remain absent instead of becoming fabricated values."""
    ohlcv = _ohlcv_with_spy()
    mock_provider = _create_mock_provider(ohlcv)
    ...
    try:
        ...
        candidate = res.json()["candidates"][0]
        assert candidate["sma_20"] is None
        assert candidate["sma_50"] is None
        assert candidate["sma_200"] is None
        assert candidate["recommendation"]["thesis"] is None
    finally:
        app.dependency_overrides.pop(get_portfolio_service, None)


def test_screener_blocks_sizing_for_unknown_currency(monkeypatch):
    """Unknown quote currency blocks execution sizing instead of assuming 1:1 FX."""
    ohlcv = _ohlcv_with_spy()
    mock_provider = _create_mock_provider(ohlcv)
    ...
    try:
        ...
        assert candidate["recommendation"]["risk"]["shares"] == 0
        assert candidate["recommendation"]["risk"]["risk_amount"] == 0.0
        reason_codes = {
...
def test_screener_coerces_nonfinite_float_fields(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_multiple_ticker_info",
        lambda tickers: {
            "AAA": {"name": "AAA Corp", "sector": "Technology", "currency": "EUR"}
        },
    )
    monkeypatch.setattr(
        screener_service,
    ...
    try:
        ...
        assert candidate["sma20_slope"] is None
    finally:
        app.dependency_overrides.pop(get_portfolio_service, None)
