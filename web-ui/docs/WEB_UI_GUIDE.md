...
| Page | Route | Purpose |
|------|-------|---------|
| Today | `/today` | Left panel with three tabs — Today (all open positions, pending orders, watchlist-near-trigger items, and a subset of one pinned screener run), Last Run (the latest screener result), Watchlist — plus the symbol analysis canvas on the right. Last Run groups candidates in this order: **Pronti per revisione ordine**, **In attesa del trigger**, **Richiede verifica**, and **Interessanti, nessun setup**. Each row shows one concrete next action from the canonical workflow state; the no-setup group is visually secondary and collapsed initially, and only ready candidates expose the manual **Rivedi ordine** action. Client lists preserve the server-provided `priorityRank`/`rank` and never reorder by the compatibility action. The Today **Ready** filter and the screener **Ready only** control both require `recommendation.workflowStatus === 'ready'`; the compatibility verdict does not qualify a row. A completed scan becomes Today's source only when **Use this run for Today's review** is selected; exploration scans leave the pinned source unchanged. For a held symbol the Order tab is hidden unless the backend returns an `ADD_ON`/`SCALE_BACK` candidate whose live-stop-adjusted workflow is still ready, and position management is folded into Overview |
