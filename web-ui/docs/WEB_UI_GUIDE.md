...
| Page | Route | Purpose |
|------|-------|---------|
| Today | `/today` | Left panel with three tabs — Today (all open positions, pending orders, watchlist-near-trigger items, and a subset of one pinned screener run), Last Run (the latest screener result), Watchlist — plus the symbol analysis canvas on the right. Selecting a symbol expands the canvas and makes the currently mounted source panel render a compact symbol rail from its same loaded collection; collapsing restores the full controls without resetting tab, filters, sorting, or query ownership. The sticky workspace header provides company/mode context, screener run date and final-close/intraday state, aggregate source health, non-intelligence bulk refresh, close, collapse, and full-screen controls, while mobile uses a back-to-list control. A source status bar reports the current snapshot of each workspace input's phase and server-owned provenance. The separate activity drawer shows only the selected ticker/version and retains up to 20 request-ID lifecycles for that session, including active, completed, partial, failed, and discarded callbacks; query fetch transitions and explicit mutations use the same lifecycle, a successful retry supersedes the older same-source failure, and only a new failure is announced as an alert. Position/order health follows the canonical open-position and order query lifecycle rather than symbol presence alone; the UI consumes server-derived ledger freshness from `app_config.portfolio_snapshot_stale_after_days` and never treats React Query refetch eligibility as domain staleness. Evidence health comes from structured refresh manifests or saved enrichment diagnostics, never from candidate-summary presence. Last Run groups candidates in this order: **Pronti per revisione ordine**, **In attesa del trigger**, **Richiede verifica**, and **Interessanti, nessun setup**. Each row shows one concrete next action from the canonical workflow state; the no-setup group is visually secondary and collapsed initially, and only ready candidates expose the manual **Rivedi ordine** action. A completed scan becomes Today's source only when **Use this run for Today's review** is selected; exploration scans leave the pinned source unchanged. For a held symbol the Order tab is hidden unless a fresh, canonically ready add-on signal exists, and position management is folded into Overview. Its advisory live preview is explicitly read-only and separated from stop, scale-out, and exit mutations. Failed order and position mutations keep the entered form values available for correction or retry |
