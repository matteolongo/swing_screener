...
| Page | Route | Purpose |
|------|-------|---------|
| Today | `/today` | Left panel with three tabs — Today (all open positions, pending orders, watchlist-near-trigger items, and a subset of one pinned screener run), Last Run (the latest screener result), Watchlist — plus the symbol analysis canvas on the right. Last Run shows execution readiness derived from the setup, trigger, and plan gates; for example, a qualified conditional entry is **Waiting for trigger**, not a failed setup. A completed scan becomes Today's source only when **Use this run for Today's review** is selected; exploration scans leave the pinned source unchanged. For a held symbol the Order tab is hidden (unless a fresh add-on entry signal exists) and position management is folded into Overview |
...
2. **Today** — check all open positions and pending orders, then review candidates from the explicitly pinned screener run. Use the Last Run checkbox to update Today's source, or clear it to explore another universe without changing the review. The **Ready only** filter keeps candidates that passed the strict recommendation checklist; conditional `Buy on Pullback` and `Wait for Breakout` actions can remain useful Today opportunities while their execution readiness says **Waiting for trigger**. Track the Watchlist and trigger symbol analysis as needed.
