...
// ─── Content behavior — ported from the former Today analysis-canvas tests ────
// Mechanical translation: set workspaceStore.selectedTicker + seed screenerStore,
// then render the drawer instead of the bare canvas. Assertion intent unchanged.

describe('SymbolDrawer — content behavior', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(watchlistHooks.useWatchlist).mockReturnValue({
      data: [],
  ...
  it('runs fundamentals analysis for the selected symbol from the drawer', async () => {
    const mutate = vi.fn();
...
describe('SymbolDrawer — compute analysis button', () => {
  beforeEach(() => {
    vi.clearAllMocks();
