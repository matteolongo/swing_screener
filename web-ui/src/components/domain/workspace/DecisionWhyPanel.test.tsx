  ...
  it('renders what-to-do / why-now / watch-for from the screener summary', () => {
    render(
      <DecisionWhyPanel
        ...
        recommendation={{ workflowStatus: 'no_setup', nextStep: { code: 'observe' } } as never}
        aiSummaryLine={null}
      />,
    );
    ...
    expect(screen.queryByText(summary.whatToDo)).not.toBeInTheDocument();
  ...
  it('appends the AI summary line to why-now when present', () => {
    render(
      <DecisionWhyPanel
        summary={summary as never}
        recommendation={{ workflowStatus: 'no_setup', nextStep: { code: 'observe' } } as never}
        aiSummaryLine="AI: momentum confirms continuation."
      />,
    );
