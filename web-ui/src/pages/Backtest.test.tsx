...
describe('Backtest Page', () => {
  beforeEach(() => {
    localStorage.setItem('backtest.params.v1', '{}')
  })

  it('renders backtest title and parameters card', async () => {
    const { queryClient } = renderWithProviders(<Backtest />)
  ...
  it('runs a backtest and shows summary', async () => {
    const { user, queryClient } = renderWithProviders(<Backtest />)
    await waitForQueriesToSettle(queryClient)
